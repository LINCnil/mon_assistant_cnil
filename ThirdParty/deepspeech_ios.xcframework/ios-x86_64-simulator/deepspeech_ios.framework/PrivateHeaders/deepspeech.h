 ...
 * @param path_to_save_wav The full filepath to save audio file
 *
...
/**
 * @brief Save audio buffer in specified folder.
 *
 * @param path_to_save_wav The full filepath to save audio file
 *
 */
DEEPSPEECH_EXPORT
void DS_saveAudio(const char* path_to_save_wav);
 ...
 *
 * @param aSctx A streaming state pointer returned by {@link DS_CreateStream()}.
 * @param path_to_save_audio Path to save audio
 *
 * @return The STT result. The user is responsible for freeing the string using
 *         {@link DS_FreeString()}.
 *
 ...
 */
DEEPSPEECH_EXPORT
char* DS_FinishStream(StreamingState* aSctx, const char* path_to_save_audio);
